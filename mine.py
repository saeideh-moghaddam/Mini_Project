...
class Login(QWindow):
    ...
    def check(self):
        ...
        if detabase.checkPassword(username, password.encode(password)):
            self.ui = MainWindow()
        else:
...
class Add_employee(QWindow):
    def __init__(self):
        super().__init__()
        loader = QUiLoader()
        self.ui = loader.load("ui/add_user.ui", None)
        self.ui.show()
...
class Effect(QWindow):
    ...
    def select(self):
        ...
        if mode==1:
            cv2.imwrite(f'img/user_image/{self.fullname}.jpeg', self.normal)
        elif mode==2:
...
class Edit_employee(QWindow):
    def __init__(self, id):
        super().__init__()
        loader = QUiLoader()
        self.ui = loader.load('ui/edit_user.ui', None)
        self.ui.show()
        self.id = id
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        loader = QUiLoader()
        self.ui = loader.load('ui/mine.ui', None)
        self.ui.show()
    ...
    def readFromDataBase(self):
        ...
        for i in range(len(result)):
            ...
            edit_btn.setIcon(QIcon('photo/user_edit.png'))
            ...
            delete_btn.setIcon(QIcon('photo/delete.png'))
